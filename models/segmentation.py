...
class DETRsegm(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        bbox_mask = self.bbox_attention(hs[-1], inter_references[-1], mask=mask)

        seg_masks = self.mask_head(srcs[-1], bbox_mask, [features[2].tensors, features[1].tensors, features[0].tensors])
...
def sigmoid_focal_loss(inputs, targets, alpha: float = 0.25, gamma: float = 2):
    ...
    # loss = F.binary_cross_entropy(inputs.sigmoid(), targets, reduction="none")
